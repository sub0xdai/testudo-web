export function Footer() {
  return (
    <footer className="relative z-10 px-6 md:px-12 lg:px-24 py-12 border-t border-container-border/30 bg-main-bg">
      <div className="max-w-5xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-6">
          <span className="font-mono text-xs text-text-tertiary">
            &copy; 2026 TESTUDO
          </span>
          <a
            href="https://github.com/sub0xdai/testudo-exchange"
            target="_blank"
            rel="noopener noreferrer"
            className="font-mono text-xs text-text-tertiary hover:text-text-primary transition-colors"
          >
            GITHUB
          </a>
          <a
            href="https://x.com/i/communities/2009337617720987685"
            target="_blank"
            rel="noopener noreferrer"
            className="font-mono text-xs text-text-tertiary hover:text-text-primary transition-colors"
          >
            COMMUNITY
          </a>
        </div>

        <div className="flex gap-6">
          <a href="#" className="font-mono text-xs text-text-tertiary hover:text-text-secondary transition-colors">
            TERMS
          </a>
          <a href="#" className="font-mono text-xs text-text-tertiary hover:text-text-secondary transition-colors">
            PRIVACY
          </a>
        </div>
      </div>
    </footer>
  )
}
